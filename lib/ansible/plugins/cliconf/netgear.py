...
cliconf: netgear
short_description: Use netgear cliconf to run command on Netgear M-series platform
description:
  - This netgear plugin provides low level abstraction apis for
    sending and receiving CLI commands from Netgear M-Series network devices.
version_added: "2.4"
"""
...
class Cliconf(CliconfBase):
    ...
    def edit_macro(self, candidate=None, commit=True, replace=None, comment=None):
        ...
        if commit:
            ...
            for line in candidate:
                if line != 'None':
                    commands += (' ' + line + '\n')
                self.send_command('configure', sendonly=True)
    ...
    def get_device_info(self):
        ...
        device_info['network_os'] = 'netgear'
        reply = self.get(command='show version')
        data = to_text(reply, errors='surrogate_or_strict').strip()

        match = re.search(r'Software Version.+ (\w.+)', data)
        if match:
            device_info['network_os_version'] = match.group(1)

        match = re.search(r'Machine Model.+ (\w.+)', data, re.M)
        if match:
